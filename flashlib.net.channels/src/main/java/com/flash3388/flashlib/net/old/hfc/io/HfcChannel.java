...
import com.flash3388.flashlib.net.data.RemoteImpl;
import com.flash3388.flashlib.net.RemotesStorage;
import com.flash3388.flashlib.net.data.RemotesStorageImpl;
...
public class HfcChannel implements Closeable {
    ...
    HfcChannel(String id, MultiTargetUdpChannel channel, KnownPacketTypes packetTypes, Clock clock, Logger logger) {
        mOurId = id;
        mChannel = channel;
        mClock = clock;
        mLogger = logger;
        ...
        mSerializer = new PacketSerializer(packetTypes);
    }

    public HfcChannel(String id, KnownPacketTypes packetTypes, Clock clock, Logger logger) {
        this(id, new MultiTargetUdpChannel(PORTS, logger), packetTypes, clock, logger);
    }
    ...
    public synchronized void handleUpdates(UpdateHandler handler) throws IOException, InterruptedException {
        ...
        while (true) {
            try {
                ...
                handler.onPacketReceived(packetInfo, packet, isFirstPacked);
