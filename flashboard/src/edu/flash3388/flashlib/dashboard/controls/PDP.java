...
public class PDP extends Displayble{
	...
	public void newData(byte[] bytes) {
		...
		if(bytes.length <= 26 && bytes[0] == 0){
			...
			allVoltage = Mathd.roundDecimal(v);
			temperature = Mathd.roundDecimal(t);
			allCurrent = Mathd.roundDecimal(c);
		}
			...
			int pos = 2;
			double v = Mathd.roundDecimal(FlashUtil.toDouble(bytes, pos)); pos+=8;
			double t = Mathd.roundDecimal(FlashUtil.toDouble(bytes, pos)); pos+=8;
			double c = Mathd.roundDecimal(FlashUtil.toDouble(bytes, pos)); pos+=8;
			
			for (int i = 0; i < channelsCurrent.length; i++) {
				channelsCurrent[i] = Mathd.roundDecimal(FlashUtil.toDouble(bytes, pos));
				pos += 8;
			}
