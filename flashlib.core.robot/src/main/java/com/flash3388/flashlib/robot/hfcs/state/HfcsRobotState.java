...
import com.flash3388.flashlib.robot.hfcs.control.ControlData;
...
public class HfcsRobotState {
    ...
    public static void registerReceiver(HfcsRegistry registry, BiConsumer<InstanceId, StateData> consumer) {
    ...
    public static Supplier<StateData> registerReceiver(HfcsRegistry registry) {
        StateData initialData = new StateData(RobotMode.DISABLED, Time.milliseconds(0));
        Property<StateData> stateDataProperty = new AtomicProperty<>(initialData);
        registry.registerIncoming(new StateDataInType())
                .addListener((event)-> {
